...
LOCATION_ID       = 1
VISIT_TYPE_ID     = 1
ENCOUNTER_TYPE_ID = 484   # adjust for Other Interventions
FORM_ID           = 552   # adjust to your form_id
CREATOR_ID        = 1
PROVIDER_ID       = 1
ENCOUNTER_ROLE_ID = 1
...
concept_map = {
    ...
    "referrals":            {"concept_id": 1001801, "type": "text"}
}
...
def insert_obs(cursor, person_id, encounter_id, concept_id, value, value_type, field_name, obs_date):
    ...
    # log
    cursor.execute("""
        INSERT INTO obs_other_intervention_log (obs_id, person_id, encounter_id, concept_id, field_name, value)
        VALUES (%s,%s,%s,%s,%s,%s)
...
def main():
    ...
    cursor.execute("""
        SELECT m.* FROM tbl_other_interventions m
    ...
    conn.commit()
    cursor.close()
    print(f"Other Interventions migrated. Obs inserted: {inserted_rows}, skipped(no patient): {skipped_no_patient}")
